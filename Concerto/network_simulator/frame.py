...
class Frame(object):
    ...
    def separate_frame_to_packet(self, start_seq, source_ip=None, destination_ip=None):
        packet_num = self.frame_size // Packet.max_payload_size
        packet_list = []
        seq = start_seq
        for i_packet in range(0, packet_num):
            packet = Packet(self.frameId, Packet.max_payload_size, seq, source_ip, destination_ip)
            packet_list.append(packet)
            # print('seq:', seq)
            seq += 1

        if self.frame_size % Packet.max_payload_size != 0:
            packet = Packet(self.frameId, self.frame_size % Packet.max_payload_size, seq, source_ip, destination_ip)
            # print('seq:', seq)
            packet_list.append(packet)
